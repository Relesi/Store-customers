...
class LocalTime extends CApplicationComponent {
    ...
    // http://php.net/manual/en/function.date.php
    public function getUTCNow($format = 'Y-m-d H:i:s', $strtime = null) {
        $utcnow = new DateTime(null, $this->serverDateTimeZone);

        if (!is_null($strtime))
            $utcnow->modify($strtime);
     ...
     * (PHP 5)
     * 
     * Converts a given date to UTC format returned on given format
     * 
     * @param String $date
     * @param String $format
     * @param String $strtime Can define an offset to date
     * @return String
     */
    public function toUTCFromDate($date, $format = null, $strtime = null) {
        $utcnow = new DateTime($this->toUTC($date), $this->serverDateTimeZone);
        
        //Date format in case it's null
        if (is_null($format))
            $format = 'Y-m-d H:i:s';
     ...
     * (PHP 5)
     * 
     * Retrives difference between two dates according to given format
     * 
     * @param String $first_date First date for comparision
     * @param String $second_date Second date for comparision
     * @return Int number days with diference of dates
     */
    public function getDateDifference($first_date, $second_date) {

            $time_ini = strtotime($first_date);
            $time_fim = strtotime($second_date);

            $diff = $time_fim - $time_ini;
    ...
    public function fromUTC($servertimestamp, $format = "Y-m-d H:i:s") {
