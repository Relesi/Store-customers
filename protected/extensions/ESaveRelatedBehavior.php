            ...
            if ($relation instanceof CManyManyRelation) {
                if (!$this->owner->isNewRecord) { // The owner also has to have saved successfully, so that the foreign key can be determined (if not, do nothing)
                    if (preg_match(// extract infos about mn linking table
                                    '/^\s*\{{0,2}\s*(.+?)\s*\}{0,2}\s*\(\s*(.+)\s*,\s*(.+)\s*\)\s*$/s', $relation->foreignKey, $matches
                            )) {
                        $info = array(
                            'mnTable' => $matches[1],
                            'mnFk1' => $matches[2],
                ...
                foreach ($data as $key => $value) {
                    ...
                    $obj->{$relation->foreignKey} = (int) $this->owner->primaryKey;
...
//                        var_dump($obj->getErrors());
            ...
            } else if ($relation instanceof CHasOneRelation) { //Handle has_one relations
                $obj = new $relation->className;
                
                if (empty($data[0]->attributes[$obj->tableSchema->primaryKey])) { //In case of a new register
                    $obj->attributes = $data[0]->attributes;
                    $obj->{$relation->foreignKey} = $this->owner->primaryKey;
                } else { //Older register, prepare for update
                    $obj = $obj->findByPk($data[0]->attributes[$obj->tableSchema->primaryKey]);
                    $obj->attributes = $data[0]->attributes;
                }
                
                if (!empty($obj->primaryKey))
                    $obj->setIsNewRecord(false);

                if ((!$this->owner->isNewRecord && !$obj->save()) || ($this->owner->isNewRecord && !$obj->validate()))
                    $result = false;
            }
        }
        if ($t && $result) {
